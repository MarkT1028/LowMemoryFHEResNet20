...
be reused; the precision fix does not require new evaluation keys. The program
saves four Stage 3 ciphertexts both before and after their final refresh:

- `checkpoints/native128-stage3-unrefreshed-v3-shard[0-3].bin` is written
  before the final normalization and bootstrap;
- `checkpoints/native128-stage3-scaled-v3-shard[0-3].bin` is written after the
  refresh and immediately before the final layer.
...
Older checkpoint names without the `scaled-v3` or `unrefreshed-v3` marker are
not loaded by these commands because they may contain the high-error ciphertext
that this revision replaces. They may be left on disk or removed manually.
...
For the 128x128 path, the final Stage 3 refresh uses OpenFHE's two-iteration
bootstrap. Before that bootstrap, Stage 3 is scaled by `1/16` and explicitly
rescaled; the positive factor is restored on the client after decryption, so it
does not change the predicted class. The global average is also scaled before
its rotate-and-add reduction. These changes prevent large Stage 3 values and
the sum of 1024 spatial positions from amplifying the CKKS approximation error.

The more expensive `probe_refresh` mode exercises the normalization,
two-iteration bootstrap, final aggregation, and numerical comparison together:
