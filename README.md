...
Run the bundled dog image with encrypted model parameters:
...
#### Native 128x128 encrypted-model-parameter experiment

Encrypted parameters are the default on this branch. The implementation
encrypts every compact Conv+BN fused diagonal, every fused bias, and each of the
four final fully connected weight partitions. Packing masks, refresh factors,
and fixed residual-path scales are circuit constants rather than learned model
parameters, so they remain plaintext.

The parameter ciphertexts are created on demand and released after use. This
preserves the existing channel-sharded packing and avoids holding tens of
thousands of parameter ciphertexts in memory at once. A complete 128x128 run
uses about 47,540 encrypted multiplicative parameter operands and 268 encrypted
bias operands, so it can take many hours.

The existing `keys_exp3_128` can be reused because it already includes the
EvalMult/relinearization key required for ciphertext-ciphertext multiplication.
Run the low-cost correctness test first:

```bash
./LowMemoryFHEResNet20 load_keys 3 resolution 128 \
  test_encrypted_weights verbose 1
```

For a controlled comparison, run the same branch, key set, and image while
changing only `weights plaintext` versus `weights encrypted`:

```bash
set -o pipefail
/usr/bin/time -v ./LowMemoryFHEResNet20 \
  load_keys 3 resolution 128 weights plaintext \
  input "inputs/dog_128x128.png" verbose 1 \
  2>&1 | tee ../logs/infer_exp3_128_plain_weights.log
...
The original final-ReLU and refresh stability fix is unchanged: degree 59 on
`[-1.25, 1.25]`, the retained `0.10` residual scale, the additional `1/16`
refresh scale, explicit rescaling, 17-bit two-iteration bootstrap, and
client-side restoration by `160`. Validate that sensitive path with encrypted
FC weights before a full run:
...
> use. The FHE operations genuinely receive ciphertext parameters, but a full
> model-confidential deployment would encrypt them offline and would not expose
> the clear `.fwgt` files to the inference process.
...
- `weights`, followed by `encrypted` or `plaintext`; this branch defaults to `encrypted` for native 64x64/128x128 inference
- `test_encrypted_weights`: run a small ciphertext-weight correctness test instead of the full inference; use it together with `load_keys`
