...
be reused; the precision fix does not require new evaluation keys. Immediately
before the final layer, the program now saves the four refreshed Stage 3
ciphertexts as `checkpoints/native128-stage3-shard0.bin` through
`native128-stage3-shard3.bin`. If the final layer is interrupted, retry only
that part instead of the hour-long CNN stages:
...
bootstrap and the global average is scaled before its rotate-and-add reduction.
Both changes reduce the CKKS approximation error that was previously amplified
while summing 1024 spatial values from four ciphertexts.
