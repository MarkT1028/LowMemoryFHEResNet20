...
Ctxt FHEController::encrypt_model_parameter(const Ptxt& parameter,
    ...
    model_parameter_encryption_time +=
...
void FHEController::print_model_parameter_stats() const {
    cout << "Model parameter mode: "
         << (encrypt_model_parameters ? "CKKS ciphertext" : "CKKS plaintext")
         << endl;
    ...
    const auto milliseconds_spent =
        duration_cast<milliseconds>(model_parameter_encryption_time).count();
    cout << "Encrypted multiplicative parameter operands: "
         << encrypted_weight_count << endl;
    cout << "Encrypted additive parameter operands: "
         << encrypted_bias_count << endl;
    cout << "On-demand model parameter encryption time: "
         << milliseconds_spent / 1000.0 << " seconds" << endl;
    cout << "Note: this preparation time is included in the timed end-to-end run."
         << endl;
}
...
EncryptedTensor FHEController::convbn_sharded(const EncryptedTensor& in,
    ...
    for (int output_shard = 0;
        ...
        Ptxt bias = sharded_bias(
            ...
            scale,
            stride2_output);
        result.shards[output_shard] =
            add_model_parameter(result.shards[output_shard], bias);
    }
